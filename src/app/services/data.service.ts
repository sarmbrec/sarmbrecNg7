...
// I would consider breaking this out, only because the methods cross resources.
// Consider a KnowledgeBaseService, a CarouselService, etc.
// This is helpful for other developers as well, in the IDE when searching for Carousel,
// they will immediately see all things 'Carousel'
...
export class DataService {
	...
	sendEmailContactForm(
		...
		console.log(
			"!!!!!!!!!!!!!!!!!!   hello from send email service !!!!!!!!!!!!!!!!!!"
		);
		return this.http.post<ContactInterface>(
			"https://formspree.io/sarmbrec@gmail.com",
			contactInterface,
			httpOptions
		);
	}
	...
	getJumbotronAsync(): Observable<JumboTron[]> {
		return this.http.get<JumboTron[]>(
			this.baseUrl + "assets/json/jumbotrons.json"
		);
	}
	...
	getCarouselListAsync(): Observable<Carousel[]> {
		return this.http.get<Carousel[]>(
			this.baseUrl + "assets/json/carousel-tech-stack.json"
		);
	}
	getCarouselItemAsync(id: string) {
		return this.getCarouselListAsync().pipe(
			map((data: Carousel[]) =>
				data.find(item => item.title.toLowerCase().replace(/ /g, "") === id)
			)
		);
	}
	...
	getFakeUsersList() {
		return this.http.get(
			"https://reqres.in/api/users?page=2&per_page=6&total=12"
		);
	}

	getFakeUsersListAsync(): Observable<FakeUsersListInterface[]> {
		return this.http.get<FakeUsersListInterface[]>(
			"https://reqres.in/api/users?page=2&per_page=6&total=12"
		);
	}

	getExperienceAsync(): Observable<ExperienceInterface[]> {
		return this.http.get<ExperienceInterface[]>(
			this.baseUrl + "assets/json/experience.json"
		);
	}

	getExperienceDetailAsync(id: string) {
		return this.getExperienceAsync().pipe(
			map((data: ExperienceInterface[]) =>
				data.find(item => item.company.toLowerCase().replace(/ /g, "") === id)
			)
		);
	}

	getExperiencesMenuAsync() {
		return this.http.get<ExperienceMenuInterface[]>(
			this.baseUrl + "assets/json/experience.json"
		);
	}

	getKnowledgeBaseAsync(): Observable<KbInterface[]> {
		return this.http.get<KbInterface[]>(
			this.baseUrl + "assets/json/knowledge-base.json"
		);
	}
	...
	getKbHtmlContent(url: string) {
		...
		return this.http.get(this.baseUrl + "assets/kbHtml/" + url, {
